...
The electric field is defined in Equation 2. If the voltage were doubled (which would be in both the \(x\) and \(y\) direction), then the overall electric field would double:
\begin{equation*}
  \begin{split}
    \vec{E} &= \left(\dfrac{\delta{V}}{\delta{x}}\hat{i} + \dfrac{\delta{V}}{\delta{y}}\hat{j} + \dfrac{\delta{V}}{\delta{z}}\hat{k}\right) \\
    \vec{E} &= \left(\dfrac{\delta{V}}{\delta{x}}\hat{i} + \dfrac{\delta{V}}{\delta{y}}\hat{j} + \cancelto{0}{\dfrac{\delta{V}}{\delta{z}}\hat{k}}\right) \\
  \end{split}
\end{equation*}
